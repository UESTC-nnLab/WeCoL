...
class MAP_vid(object):
    _defaults = {
        ...
        #   如果出现shape不匹配，同时要注意训练时的model_path和classes_path参数的修改c
        #--------------------------------------------------------------------------#
        "model_path"        : '/home/dww/OD/weak_stream5/logs/loss_2025_03_13_18_54_49/ep024-loss0.952-val_loss1.594.pth', 
        #/home/dww/OD/weak_stream5/logs/loss_2025_03_08_12_10_39/ep023-loss1.473-val_loss1.905.pth
...
if __name__ == "__main__":
    ...
    if map_mode == 0 or map_mode == 1:
        yolo = MAP_vid(confidence = 0.001, nms_iou = 0.65) # confidence
        ...
        with open(os.path.join(temp_save_path, 'eval_results.json'),"w") as f:
            ...
            json.dump(results, f)
            ####将results中的结果按照image_id处理后写进txt文件
            # image_bboxes = defaultdict(list)
            # for item in results:
            #     image_id = item['image_id']
            #     x1, y1, w, h = item['bbox']
            #     x2, y2 = x1 + w, y1 + h  # 计算 x2, y2
            #     if w > 15 or h > 15 or x1 < 0 or y1 < 0 or x2 < 0 or y2 < 0:
            #         continue
            #     bbox_int = [int(x1), int(y1), int(x2), int(y2), 0]
            #     image_bboxes[image_id].append(bbox_int)
            # with open("pse_DAUB_train1.txt", "w") as f:
            #     for image_id, bboxes in image_bboxes.items():
            #         image_path  = os.path.join(dataset_img_path, cocoGt.loadImgs(image_id)[0]['file_name'])
            #         bbox_strings = ["{},{},{},{},{}".format(*bbox) for bbox in bboxes]  # 转换 bbox 为字符串
            #         line = image_path + " " + " ".join(bbox_strings)  # 组合完整行
            #         f.write(line + "\n")
            # print("数据已写入 bboxes.txt")
