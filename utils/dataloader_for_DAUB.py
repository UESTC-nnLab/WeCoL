...
class seqDataset(Dataset):
    def __init__(self, dataset_path, image_size, num_frame=5):
        ...
        self.dataset_name = dataset_path.split('/')[-1].split('_')[1] # pse_IRDST_train.txt
        self.aug = True
        with open(self.txt_path) as f: 
            ...
            for line in data_lines:
                ...
                self.anno_idx.append(np.array([np.array(list(map(int,box.split(',')))) for box in line[2:]]))
                self.obj_num.append(int(line[1])) # add target number of each image
    ...
    def __getitem__(self, index):
        ...
        return images, box, self.obj_num[index], self.img_idx[index]
    ...
    def get_data(self, index):
        ...
        for id in range(0, self.num_frame):
            ...
            if len(label_data) > 0 and id == 0: #只保存最后一帧的位置，关键帧
                # np.random.shuffle(label_data)
                if self.dataset_name == 'IRDST':
                    label_data = label_data[label_data[:,2]-label_data[:,0]<20]
                    label_data = label_data[label_data[:,3]-label_data[:,1]<15]
                elif self.dataset_name == 'DAUB':
                    label_data = label_data[label_data[:,2]-label_data[:,0]<15]
                    label_data = label_data[label_data[:,3]-label_data[:,1]<15]
                elif self.dataset_name == 'ITSDT':
                    label_data = label_data[label_data[:,2]-label_data[:,0]<25]
                    label_data = label_data[label_data[:,3]-label_data[:,1]<15]
...
def dataset_collate_train(batch):
    ...
    obj_num = []
    img_path = []
    for img, box, objnum, imgpath in batch:
        images.append(img)
        bboxes.append(box)
        obj_num.append(objnum)
        img_path.append(imgpath)
    images = torch.from_numpy(np.array(images)).type(torch.FloatTensor)
    bboxes = [torch.from_numpy(ann).type(torch.FloatTensor) for ann in bboxes]
    obj_num = [torch.tensor([ann]) for ann in obj_num]
    return images, bboxes, obj_num, img_path
    # return images, bboxes

class seqValDataset(Dataset):
    def __init__(self, dataset_path, image_size, num_frame=5):
        super(seqValDataset, self).__init__()
        self.dataset_path = dataset_path
        self.img_idx = []
        self.anno_idx = []
        self.obj_num = []
...
if __name__ == "__main__":
    ...
    train_dataloader = DataLoader(train_dataset, shuffle=True, batch_size=2, collate_fn=dataset_collate_val)
    t = time.time()
