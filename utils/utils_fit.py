...
def fit_one_epoch(model_train, model, ema, yolo_loss, loss_history, eval_callback, optimizer, epoch, epoch_step, epoch_step_val, gen, gen_val, Epoch, cuda, fp16, scaler, save_period, save_dir, local_rank=0):
    ...
    for iteration, batch in enumerate(gen):
        ...
        images, targets, obj_nums = batch[0], batch[1], batch[2]
        with torch.no_grad():
            if cuda:
        ...
        if not fp16:
            ...
            #   前向传播
            #----------------------#
            outputs, new_targets, milloss = model_train(images, targets, obj_nums)


            #----------------------#
            #   计算损失
            #----------------------#
            loss_value = yolo_loss(outputs, new_targets)
            
            loss_value = loss_value + milloss#####ratio
            ...
            with autocast():
                ...
                outputs, new_targets, milloss = model_train(images, targets, obj_nums, type='train')
                #----------------------#
                #   计算损失
                #----------------------#
                loss_value = yolo_loss(outputs, new_targets)
    ...
    for iteration, batch in enumerate(gen_val):
        ...
        with torch.no_grad():
            ...
            outputs         = model_train_eval(images, type='val')
