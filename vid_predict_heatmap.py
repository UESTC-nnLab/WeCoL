...
class Pred_vid(object):
    _defaults = {
        ...
        #--------------------------------------------------------------------------#
        "model_path"        : '/home/dww/OD/weak_stream5/logs/result/loss_2025_03_11_21_08_13ITSDT72/ep033-loss1.020-val_loss1.917.pth', #/home/dww/OD/zip/Exper_Results/ITSDT_results/Tridos/ep016-loss0.588-val_loss0.962_ITSDT8091_9065.pth
        
        "classes_path"      : '/home/dww/OD/two_stream1/model_data/classes.txt',
        #---------------------------------------------------------------------#
        #   输入图片的大小，必须为32的倍数。
        #---------------------------------------------------------------------#
        "input_shape"       : [512, 512],
    ...
    def detect_image(self, images, crop = False, count = False):
        ...
        with torch.no_grad():
            ...
            # print(self.net)
            hook_layers = ['module.backbone.C3_p3','model.neck.agpf','module.neck.conv_fre','module.neck.conv_cr_mix'
                           'module.neck.nolocal', 'module.neck.swin', 'module.neck.conv_t', 'module.neck.agpf','model.head.cls_convs'
                           'module.neck.conv_fre','module.neck.resblock0','module.neck.resblock1','module.neck.resblock2','module.neck.tdms','module.head']
            register_hook(self.net, for_hook, hook_layers)
            ################################################################
            outputs = self.net(inputs=images,type='val')
            ###########################
            i = 0
            for fea in layer_outputs:
                ...
                src_size = (320,240)  # 原图尺寸大小 (256,256)
        ...
        #   设置字体与边框厚度
        #---------------------------------------------------------#
        font        = ImageFont.truetype(font='/home/dww/OD/two_stream1/model_data/simhei.ttf', size=np.floor(3e-2 * c_image.size[1] + 15).astype('int32'))  
